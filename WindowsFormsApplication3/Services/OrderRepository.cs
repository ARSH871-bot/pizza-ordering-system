...
{
    /// <summary>
    /// Persists and retrieves <see cref="OrderRecord"/> instances using SQLite.
    /// </summary>
    public class OrderRepository : IOrderRepository
    {
        ...
        public OrderRepository(string dataDirectory)
        {
            ...
            DatabaseMigrator.Run(dataDirectory);
        ...
        public void Save(OrderRecord record)
        {
            ...
            record.Status = string.IsNullOrWhiteSpace(record.Status) ? "Active" : record.Status;
            ...
            using (var tx = conn.BeginTransaction())
            {
                ...
                if (record.Lines != null && record.Lines.Count > 0)
                {
                    conn.Execute(
                        "INSERT INTO OrderLines (OrderId, Item, Quantity, Price) VALUES (@OrderId, @Item, @Quantity, @Price)",
                        record.Lines.Select(l => new
                        {
                            OrderId = record.Id,
                            l.Item,
                            l.Quantity,
        ...
        public List<OrderRecord> LoadAll()
        {
            using (var conn = OpenConnection())
            {
                ...
                return LoadOrderLines(conn, orders);
            }
        }
        ...
        public void Delete(string id)
        {
            ...
            using (var tx = conn.BeginTransaction())
            {
                conn.Execute("DELETE FROM OrderLines WHERE OrderId = @Id", new { Id = id }, tx);
        ...
        public List<TopItem> GetTopItems(DateTime? from, DateTime? to, int limit)
        {
            string dateFilter = from.HasValue || to.HasValue
                ? "WHERE o.OrderDate >= @From AND o.OrderDate < @To AND (o.Status IS NULL OR o.Status = 'Active') "
        ...
        private static List<OrderRecord> LoadOrderLines(IDbConnection conn, List<OrderRecord> orders)
        {
            if (orders == null || orders.Count == 0)
                return orders ?? new List<OrderRecord>();

            var ids = orders.Select(o => o.Id).ToArray();
            var lines = conn.Query<LineRow>(
                "SELECT OrderId, Item, Quantity, Price FROM OrderLines WHERE OrderId IN @Ids",
                new { Ids = ids }).ToList();
            ...
            {
                if (grouped.TryGetValue(order.Id, out var orderLines))
                    order.Lines = orderLines;
                else
                    order.Lines = new List<OrderLineRecord>();

                order.Subtotal = Math.Round(order.Subtotal, 2);
                order.Tax = Math.Round(order.Tax, 2);
                order.Total = Math.Round(order.Total, 2);
                order.Discount = Math.Round(order.Discount, 2);
                order.Status = string.IsNullOrWhiteSpace(order.Status) ? "Active" : order.Status;
            }
        ...
        private void MigrateFromNdjson(string ndjsonPath)
        {
            try
            {
                ...
                foreach (string line in File.ReadLines(ndjsonPath))
                {
                    ...
                    {
                        // Skip corrupted lines and continue migrating what we can.
                    }
                }
            ...
            {
                // Legacy migration is best-effort and should never block app startup.
            }
        }
        ...
        private void MigrateFromJsonArray(string jsonPath)
        {
            try
            {
                ...
                var records = serialiser.Deserialize<List<OrderRecord>>(json) ?? new List<OrderRecord>();
